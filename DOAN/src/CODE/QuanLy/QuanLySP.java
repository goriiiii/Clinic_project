...
public class QuanLySP implements QuanLy {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (sp.xuatLoaiSanPham() != "") {
                    sp.nhapTuFile(values[1], values[2], Integer.parseInt(values[3]), Double.parseDouble(values[4]),
                            nsx);
                    dsdp.add(sp);
                }
            }
        ...
        for (int i = 0; i < dsdp.size(); i++) {
            if (dsdp.get(i).xuatMaSanPham().equals(maSo)) {
                ...
                System.out.printf("|%-15s|%-30s|%-20s|%-20s|%-30s|%-15s|%-15s|\n", "Mã sản phẩm", "Tên sản phẩm",
                        "Loại", "Mã nhà sản xuất", "Tên nhà sản xuất", "Đơn giá", "Số lượng", "Đơn giá");
                System.out.println(
                        "+---------------+------------------------------+--------------------+--------------------+------------------------------+---------------+---------------+");
                xuatSP(dsdp.get(i));
                ...
                if (Nhap.nhapXacNhan()) {
                    ...
                    System.out.println("[Thông báo]> Đã hủy sửa sản phẩm");
                }
                return;
            }
        }
        ...
        for (int i = 0; i < dsdp.size(); i++) {
            if (dsdp.get(i).xuatMaSanPham().equals(maSo)) {
                ...
                if (Nhap.nhapXacNhan()) {
                    ...
                    System.out.println("[Thông báo]> Đã hủy thanh lý sản phẩm");
                return;
            }
        }
        ...
        for (int i = 0; i < dsdp.size(); i++) {
            if (dsdp.get(i).xuatMaSanPham().equals(maSo)) {
                ...
                System.out.println(
                        "+---------------+------------------------------+--------------------+--------------------+------------------------------+---------------+---------------+");
                System.out.printf("|%-15s|%-30s|%-20s|%-20s|%-30s|%-15s|%-15s|\n", "Mã sản phẩm", "Tên sản phẩm",
                        "Loại", "Mã nhà sản xuất", "Tên nhà sản xuất", "Đơn giá", "Số lượng");
                System.out.println(
                        "+---------------+------------------------------+--------------------+--------------------+------------------------------+---------------+---------------+");
                xuatSP(dsdp.get(i));
                return;
    ...
    public static void xuatDSSP() {
        ...
        if (dsdp.isEmpty())
            ...
            System.out.println(
                    "+---------------+------------------------------+--------------------+--------------------+------------------------------+---------------+---------------+");
            System.out.printf("|%-15s|%-30s|%-20s|%-20s|%-30s|%-15s|%-15s|\n", "Mã sản phẩm", "Tên sản phẩm", "Loại",
                    "Mã nhà sản xuất", "Tên nhà sản xuất", "Đơn giá", "Số lượng");
            System.out.println(
                    "+---------------+------------------------------+--------------------+--------------------+------------------------------+---------------+---------------+");
            for (var sp : dsdp) {
                xuatSP(sp);
    ...
    // -----tự động thêm mã sản phẩm khi thêm sản phẩm (không phải chức
    // năng)--------
    public static String themMaSanPham() {
        ...
        for (i = 0; i < dsdp.size(); i++) {
            ...
            for (int j = 0; j < dsdp.size(); j++) {
                if (dsdp.get(j).xuatMaSanPham().equals("SP" + Integer.toString(i)))
                    count++;
            }
            if (count == 0)
                break;
    ...
    public static boolean kiemTraSoLuong(SanPham sanPham) {
        ...
        for (int i = 0; i < dsdp.size(); i++) {
            if (sanPham.xuatMaSanPham().equals(dsdp.get(i).xuatMaSanPham())) {
                if (dsdp.get(i).xuatSoLuong() == 0)
                    return false;
                if (sanPham.xuatSoLuong() <= dsdp.get(i).xuatSoLuong())
                    return true;
                else
                    return false;
            }
    ...
    public static void xuatSP(SanPham sp) {
        System.out.printf("|%-15s|%-30s|%-20s|%-20s|%-30s|%,-15.0f|%,-15d|\n", sp.xuatMaSanPham(), sp.xuatTenSanPham(),
                sp.xuatLoaiSanPham(), sp.xuatMaNSX(), sp.xuatTenNSX(), sp.xuatDonGia(), sp.xuatSoLuong());
        System.out.println(
                "+---------------+------------------------------+--------------------+--------------------+------------------------------+---------------+---------------+");
    }
